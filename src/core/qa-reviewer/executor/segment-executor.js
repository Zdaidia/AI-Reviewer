...
请开始审查并输出包含 requirementMatching、contractChecking、optimization 和 issues 的完整 JSON 格式结果。`
    ...
    for (let turn = 1; turn <= maxTurns; turn++) {
      ...
      const isFinalTurn = turn >= FINAL_TURN && useFunctionCalling;
      ...
      // 最后几轮禁用工具，强制输出结论
      if (useFunctionCalling && tools && !isFinalTurn) {
      ...
      // 检测连续空内容工具调用（AI 死循环保护）
      const hasContent2 = response.content && response.content.length > 0;
      if (!hasContent2) {
        consecutiveEmptyToolCalls++;
        console.log(`[SegmentExecutor] 阶段2 连续空内容工具调用: ${consecutiveEmptyToolCalls}/${MAX_EMPTY_TOOL_CALLS}`);
      } else {
        consecutiveEmptyToolCalls = 0;
      }

      if (consecutiveEmptyToolCalls >= MAX_EMPTY_TOOL_CALLS) {
        console.warn('[SegmentExecutor] 阶段2 AI 连续多轮只调工具不输出内容，强制结束并要求总结');
        messages.push({
          role: 'user',
          content: '你已进行了多轮工具调用。请立即停止调用工具，直接输出你的分析结论（JSON 格式）。不要再调用任何工具。'
        });
        try {
          const finalResp = await this.callLLMWithRetry('code_analysis', messages, {
            temperature: 0.3, maxTokens: 16000
          });
          if (finalResp.success && finalResp.content) {
            finalContent = finalResp.content;
          }
        } catch (e) { /* 忽略 */ }
        if (!finalContent) {
          finalContent = '审查完成，但 AI 未能生成最终分析结论。';
        }
        break;
      }
    ...
    if (!finalContent) {
      // 跑满轮次但 AI 没有输出文本结论，做一次强制总结调用
      console.warn('[SegmentExecutor] 阶段2 达到最大轮次且 AI 未输出结论，做强制总结');
      messages.push({
        role: 'user',
        content: '你已完成了所有文件分析。请立即根据已读取的内容输出你的最终审查结论（JSON 格式），不要再调用任何工具。'
      });
      try {
        const phase2Config = QAModelConfig.getTwoPhaseConfig(2);
        const finalResponse = await this.callLLMWithRetry('code_analysis', messages, {
          temperature: phase2Config.temperature,
          maxTokens: phase2Config.maxTokens,
          model: phase2Config.preferred,
          enableModelFallback: true
...
4. 已发现的问题已整理完毕`;
    ...
    for (let turn = 1; turn <= maxTurns; turn++) {
      ...
      const requestOptions = {
        temperature: 0.3,
        maxTokens: 16000,
        tools: isFinalTurn ? undefined : tools, // 最后几轮不提供工具，强制输出结论
      ...
      while (retryCount <= maxRetries) {
        ...
        try {
          response = await Promise.race([
            this.llm.chat('code_analysis', messages, requestOptions),
            timeoutPromise
          ]);
        } catch (timeoutErr) {
          const isTimeout = timeoutErr.message === 'API_TIMEOUT';
          const errMsg = isTimeout
            ? `AI 响应超时（超过 ${Math.round(API_TIMEOUT / 60000)} 分钟无响应），请检查网络或更换模型后重试`
            : timeoutErr.message;
          _dl('[checkRequirementsWithTools] 请求异常: ' + errMsg);
      ...
      if (!hasContent) {
        consecutiveEmptyToolCalls++;
        console.log(`[SegmentExecutor] 连续空内容工具调用: ${consecutiveEmptyToolCalls}/${MAX_EMPTY_TOOL_CALLS}`);
      } else {
        consecutiveEmptyToolCalls = 0;
      }

      if (consecutiveEmptyToolCalls >= MAX_EMPTY_TOOL_CALLS) {
        console.warn('[SegmentExecutor] AI 连续多轮只调工具不输出内容，强制结束并要求总结');
        // 发送最后一条消息要求 AI 总结
        messages.push({
          role: 'user',
          content: '你已进行了多轮工具调用。请立即停止调用工具，直接输出你的分析结论（JSON 格式）。不要再调用任何工具。'
        ...
        try {
          const finalResponse = await Promise.race([
            this.llm.chat('code_analysis', messages, {
              temperature: 0.3,
              maxTokens: 16000,
            }),
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error('API_TIMEOUT')), 2 * 60 * 1000)
            )
          ]);
          if (finalResponse.success && finalResponse.content) {
            finalContent = finalResponse.content;
          ...
          _dl('[checkRequirementsWithTools] 强制总结请求失败: ' + e.message);
        }
        if (!finalContent) {
          finalContent = '审查完成，但 AI 未能生成最终分析结论。';
        }
    ...
    if (!finalContent) {
      // 跑满轮次但 AI 没有输出文本结论，做一次强制总结调用
      console.warn('[SegmentExecutor] 达到最大轮次且 AI 未输出结论，做最后一次强制总结调用');
      messages.push({
        role: 'user',
        content: '你已完成了所有文件分析。请立即根据已读取的内容输出你的最终审查结论（JSON 格式），不要再调用任何工具。'
      });
      try {
        const finalResponse = await this.llm.chat('code_analysis', messages, {
          temperature: 0.3,
          maxTokens: 16000,
        });
        if (finalResponse.success && finalResponse.content) {
        ...
        _dl('[checkRequirementsWithTools] 强制总结调用失败: ' + e.message);
      }
    }
  ...
  cleanJsonString(jsonStr) {
    ...
    // 注意：JSON 标准不支持注释，所有 // 和 /* */ 出现的位置都在字符串值内
    // 不能盲目移除，否则会破坏 URL（如 https://...）等内容
    // 以下是仅在非字符串区域内移除注释的安全版本

    // 安全移除注释：只在 JSON 字符串值外部移除 // 和 /* */ 注释
    // 通过逐字符扫描，跳过字符串值内的内容
    function removeCommentsOutsideStrings(str) {
      let result = '';
      let inString = false;
      let escapeNext = false;
      let i = 0;
    ...
    cleaned = removeCommentsOutsideStrings(cleaned);

    // 安全移除尾随逗号：只在字符串值外部的 ] 和 } 前移除逗号
    cleaned = cleaned.replace(/,\s*([}\]])/g, function(match, bracket, offset) {
      // 检查逗号是否在字符串值内部
      let inStr = false;
      let esc = false;
      for (let j = 0; j < offset; j++) {
        if (esc) { esc = false; continue; }
        if (cleaned[j] === '\\' && inStr) { esc = true; continue; }
        if (cleaned[j] === '"') { inStr = !inStr; continue; }
      }
      // 只在字符串外移除尾随逗号
      return inStr ? match : bracket;
    });

    // 修复非法 JSON 转义字符（仅在字符串值内部）
    // 逐字符扫描，只在字符串内替换非法转义
    let escapeFixed = '';
    inString = false;
    escapeNext = false;
    for (let i = 0; i < cleaned.length; i++) {
      ...
      if (ch === '\\' && inString) {
        ...
        if (nextCh && !['"', '\\', '/', 'b', 'f', 'n', 'r', 't', 'u'].includes(nextCh)) {
          // 非法转义：\X → \\X
          escapeFixed += '\\\\';
          escapeNext = false;
          continue;
    ...
    for (let i = 0; i < cleaned.length; i++) {
      const ch = cleaned[i];
      if (escapeNext) { escapeNext = false; unicodeFixed += ch; continue; }
      if (ch === '\\' && inString) { escapeNext = true; unicodeFixed += ch; continue; }
      if (ch === '"') { inString = !inString; unicodeFixed += ch; continue; }
      if (inString && ch === 'u' && cleaned[i - 1] === '\\') {
        // 检查后面是否是4位十六进制
        const hexPart = cleaned.substring(i + 1, i + 5);
        if (!/^[0-9a-fA-F]{4}$/.test(hexPart)) {
          unicodeFixed += '\\u'; // 将 \u 转义为 \\u
          continue;
        }
      }
      unicodeFixed += ch;
    }
    cleaned = unicodeFixed;

    // 清理控制字符（保留 \t \n \r，移除其他控制字符）
    // 注意：\t(0x09) \n(0x0A) \r(0x0D) 在 JSON 字符串值内也必须被转义，
    // 但如果是行间缩进/换行则合法。这里只移除明显非法的控制字符。
  ...
  parseResponse(response) {
    ...
    try {
      ...
      // 先尝试直接解析原始 JSON（cleanJsonString 可能破坏有效 JSON）
      let result = null;
      try {
        result = JSON.parse(jsonStr);
        console.log('[SegmentExecutor] 原始 JSON 直接解析成功，跳过清理');
      } catch (rawError) {
        console.warn('[SegmentExecutor] 原始 JSON 解析失败:', rawError.message, '→ 应用清理后重试');
        // 清理 JSON 字符串
        jsonStr = this.cleanJsonString(jsonStr);
        console.log('[SegmentExecutor] 清理后 JSON 前 300 字符:', jsonStr.substring(0, 300));

        // 尝试解析（带自修复）
        result = this.parseJsonWithRepair(jsonStr);
      }
