...
// API Key 由用户在设置面板配置，不使用硬编码默认值

const { DATA_DIR } = require('../config/data-dir');

// DynamicFileMatcher 实例缓存（按项目路径）
const matcherCache = new Map();

/**
 * 获取或创建 DynamicFileMatcher 实例（带缓存）
 */
async function getMatcher(projectPath) {
  const cached = matcherCache.get(projectPath);
  if (cached && !cached.isStale()) {
    return cached;
  }
  const DynamicFileMatcher = require('../qa-reviewer/matcher/dynamic-file-matcher');
  const matcher = new DynamicFileMatcher();
  await matcher.initialize(projectPath);
  matcherCache.set(projectPath, matcher);
  return matcher;
}
...
// Import core modules - 逐个加载，允许部分失败
let CodeScanner, TodoManager, AIFixer, ProjectRunner, TestingManager;
let DependencyLoader, AITestAgent, TestReporter, TestCaseStorage, BDDTestParser, AIContextGenerator;

try { CodeScanner = require('../scanner/index'); debugLog('✓ CodeScanner'); } catch(e) { debugLog(`❌ CodeScanner: ${e.message}`); }
try { TodoManager = require('../todo/index'); debugLog('✓ TodoManager'); } catch(e) { debugLog(`❌ TodoManager: ${e.message}`); }
try { AIFixer = require('../ai-fixer/index'); debugLog('✓ AIFixer'); } catch(e) { debugLog(`❌ AIFixer: ${e.message}`); }
try { ProjectRunner = require('../runner/index'); debugLog('✓ ProjectRunner'); } catch(e) { debugLog(`❌ ProjectRunner: ${e.message}`); }
try { TestingManager = require('../testing/index'); debugLog('✓ TestingManager'); } catch(e) { debugLog(`❌ TestingManager: ${e.message}`); }
try { DependencyLoader = require('../dependency/index'); debugLog('✓ DependencyLoader'); } catch(e) { debugLog(`❌ DependencyLoader: ${e.message}`); }
try { AITestAgent = require('../testing/ai-test-agent'); debugLog('✓ AITestAgent'); } catch(e) { debugLog(`❌ AITestAgent: ${e.message}`); }
try { TestReporter = require('../testing/test-reporter'); debugLog('✓ TestReporter'); } catch(e) { debugLog(`❌ TestReporter: ${e.message}`); }
try { TestCaseStorage = require('../testing/test-case-storage'); debugLog('✓ TestCaseStorage'); } catch(e) { debugLog(`❌ TestCaseStorage: ${e.message}`); }
...
function initializeModules() {
  ...
  if (DependencyLoader) dependencyLoader = new DependencyLoader();

  // Initialize Test Reporter
  if (TestReporter) testReporter = new TestReporter({ outputDir: path.join(DATA_DIR, 'test-reports') });

  // Initialize BDD Test Parser
  if (BDDTestParser) bddTestParser = new BDDTestParser();

  // Initialize Test Case Storage
  if (TestCaseStorage) testCaseStorage = new TestCaseStorage();
...
function initializeAgent() {
  try {
    ...
    // 根据用户选择的模型，找到对应 provider 的配置
    // providerConfigs 中可能有 claude-code、zhipu 等多个 provider 的独立配置
    const providerConfigs = savedAIConfig.providerConfigs || {};
    let activeApiKey = savedAIConfig.apiKey;
    let activeApiEndpoint = savedAIConfig.apiEndpoint;

    // 根据模型的 provider 属性匹配配置
    try {
      ...
      if (modelSpec && modelSpec.provider) {
        ...
        const providerCfg = providerConfigs[providerName];
        if (providerCfg && providerCfg.apiKey && providerCfg.apiEndpoint) {
          ...
          debugLog(`[Main] 模型 ${userModel} → provider ${providerName} → endpoint ${providerCfg.apiEndpoint}`);
        }
      }
    } catch (e) {
      console.warn('[Main] 无法匹配 provider 配置:', e.message);
    }

    // 使用用户配置的 API Key 和 Endpoint（优先级最高）
    if (activeApiKey && activeApiEndpoint) {
      process.env.ANTHROPIC_AUTH_TOKEN = activeApiKey;
      process.env.ANTHROPIC_BASE_URL = activeApiEndpoint;
      console.log(`[Main] 使用用户配置: ${activeApiEndpoint}, 模型: ${userModel}`);
    } else {
      // 回退到默认智谱配置（无 Key，提示用户配置）
      console.log('[Main] 未检测到用户保存的 API 配置，请在 Settings 中配置');
    }
    ...
    // 同步智谱 API Key
    if (!hasZhipu && process.env.ANTHROPIC_AUTH_TOKEN) {
      process.env.ZHIPUAI_API_KEY = process.env.ANTHROPIC_AUTH_TOKEN;
      hasZhipu = true;
    }
    ...
    agentLLM = new LLMRouter({
      ...
      // 使用匹配到对应 provider 的配置
      projectApiKey: activeApiKey,
      projectApiEndpoint: activeApiEndpoint
    });
...
ipcMain.handle('ai-fix-update-config', async (event, updates) => {
  try {
    ...
    if (global.llmRouter) {
      const routerUpdates = {};
      const userModel = updates.model || 'glm-5.1';
      if (userModel) {
      ...
      // 根据模型的 provider 找到对应配置
      let activeApiKey = updates.apiKey;
      let activeApiEndpoint = updates.apiEndpoint;
      try {
        const { MODEL_SPECS } = require('../agent/llm/router');
        const modelSpec = MODEL_SPECS[userModel];
        if (modelSpec && modelSpec.provider) {
          const fullConfig = aiFixer?.config?.getConfig() || {};
          const providerCfg = (fullConfig.providerConfigs || {})[modelSpec.provider];
          if (providerCfg && providerCfg.apiKey && providerCfg.apiEndpoint) {
            activeApiKey = providerCfg.apiKey;
            activeApiEndpoint = providerCfg.apiEndpoint;
            debugLog(`[ai-fix-update-config] 模型 ${userModel} → provider ${modelSpec.provider} → endpoint ${providerCfg.apiEndpoint}`);
          }
        }
      } catch (e) { /* 忽略 */ }
...
app.whenReady().then(() => {
  ...
  debugLog(`getAppRoot: ${getAppRoot()}`);
  ...
  debugLog('开始 initializeModules...');
  initializeModules();
  debugLog('initializeModules 完成');
...
function groupFilesByModule(filePaths, params) {
  ...
  // 1. 首先按文件角色分类（而不是按模块）
  const coreFiles = [];       // 主要文件：view, controller, api（必须全文读取）
  const referenceFiles = [];  // 参考文件：model, binding, service（用摘要即可）
  ...
  for (const filePath of filePaths) {
    ...
    if (['view', 'controller', 'api'].includes(role)) {
      coreFiles.push({ filePath, role });
    } else if (['model', 'binding', 'service'].includes(role)) {
      referenceFiles.push({ filePath, role });
  ...
  console.log(`[groupFilesByModule] 文件分类: core=${coreFiles.length}, reference=${referenceFiles.length}, components=${componentFiles.length}, i18n=${i18nFiles.length}, other=${otherFiles.length}`);
  ...
  if (totalCoreFiles <= MAX_FILES_PER_SEGMENT) {
    ...
    const allReferenceFilePaths = referenceFiles.map(f => f.filePath);
    const allComponentFilePaths = componentFiles.map(f => f.filePath);
    const allOtherFilePaths = otherFiles.map(f => f.filePath);

    // 区分共享文件和模块参考文件
    const allSharedFilePaths = referenceFiles.filter(f => isSharedFile(f.filePath, params.projectType || 'flutter')).map(f => f.filePath);
    const allModuleRefFilePaths = referenceFiles.filter(f => !isSharedFile(f.filePath, params.projectType || 'flutter')).map(f => f.filePath);

    // 创建一个包含所有主要文件的分段
    const segment = new Segment({
      name: '完整功能审查',
      description: `审查所有主要文件（${totalCoreFiles}个）+ 组件按需读取`,
      files: [...allCoreFilePaths, ...allModuleRefFilePaths, ...allOtherFilePaths],
      features: ['full-review'],
      // 元数据：告诉执行器有哪些组件、参考文件和共享文件
      metadata: {
        availableComponents: allComponentFilePaths,
        componentCount: allComponentFilePaths.length,
        referenceFiles: allReferenceFilePaths,
        referenceCount: allReferenceFilePaths.length,
        sharedFiles: allSharedFilePaths,
        sharedCount: allSharedFilePaths.length,
        i18nFiles: i18nFilePaths,
        i18nCount: i18nFilePaths.length
      }
    });

    console.log(`[groupFilesByModule] 创建1个分段，包含${totalCoreFiles}个主要文件，${allReferenceFilePaths.length}个参考文件(${allSharedFilePaths.length}共享)，${allComponentFilePaths.length}个组件可用，${i18nFilePaths.length}个多语言文件可用`);
    return [segment];
  }
...
function inferFileType(filePath, projectType = 'flutter') {
  ...
  if (projectType === 'flutter') {
    ...
    // Vue/React 项目中的 binding/store/composables
    if (normalizedPath.includes('/composables/') || normalizedPath.includes('/hooks/')) {
      return 'binding';
    }
 ...
 * 共享文件通常不属于特定功能模块，而是被多个模块引用
 */
function isSharedFile(filePath, projectType = 'flutter') {
  const normalizedPath = filePath.replace(/\\/g, '/').toLowerCase();
  const fileName = filePath.split(/[/\\]/).pop().toLowerCase();
...
ipcMain.handle('qa-reviewer:execute', async (event, params) => {
  ...
  try {
    ...
    if (params.selectedFiles && params.selectedFiles.length > 0) {
      ...
      // 使用 DynamicFileMatcher 查找相关文件（动态匹配，不使用硬编码映射）
      const relatedFiles = new Set();

      try {
        const matcher = await getMatcher(projectPath);

        for (const module of params.selectedModules) {
          const result = matcher.matchFilesFromText(module);
          if (result.files.length > 0) {
            console.log(`[QA Reviewer] 模块"${module}"匹配到 ${result.files.length} 个文件 (method: ${result.method})`);
            result.files.forEach(f => relatedFiles.add(f.path));
          } else {
            console.log(`[QA Reviewer] 模块"${module}"未匹配到文件`);
          }
        }
      } catch (matcherError) {
        console.warn('[QA Reviewer] DynamicFileMatcher 失败，回退到代码图搜索:', matcherError.message);

        // 回退：从模块名提取关键词搜索代码图
        const allKeywords = new Set();
        params.selectedModules.forEach(module => {
          allKeywords.add(module.toLowerCase());
          allKeywords.add(module.toLowerCase().replace(/\s+/g, '_'));
          // 提取中文短语
          const phrases = module.match(/[一-鿿]{2,6}/g) || [];
          phrases.forEach(p => allKeywords.add(p));
        });
        ...
        codeGraph.nodes.forEach(node => {
          if (node.type === 'file' || node.file) {
            ...
            if (isMatch) relatedFiles.add(filePath);
          }
        });
      }
    ...
    const executor = new SegmentExecutor({
      qaReviewer: reviewer,
      llm: global.llmRouter || null,
      codeScanner: null,  // AI QA Reviewer 不使用传统规则扫描
      deepAnalyzer: null,  // AI QA Reviewer 不使用深度分析
    ...
    // 汇总结果
    debugLog('[qa-reviewer:execute] 审查执行完成，开始汇总结果');
    const successfulResults = results.filter(r => r.success && r.results);
    debugLog(`[qa-reviewer:execute] 成功分段: ${successfulResults.length}/${results.length}`);
    ...
    debugLog('[qa-reviewer:execute] 执行异常: ' + error.message + '\n' + error.stack);
...
/**
 * 通过页面名称匹配文件
 * 优先使用 DynamicFileMatcher（完整的 i18n→menu→route→view 追踪链路）
 * 回退到代码图关键词搜索
 */
ipcMain.handle('qa-reviewer:search-files-by-page-name', async (event, params) => {
  debugLog('[searchFilesByPageName] 收到请求: ' + JSON.stringify(params));
  try {
    ...
    console.log('[searchFilesByPageName] 搜索:', pageName, '项目:', projectPath);

    // 1. 优先使用 DynamicFileMatcher（i18n→menu→route→view 完整追踪链路）
    try {
      debugLog('[searchFilesByPageName] 开始 getMatcher...');
      const matcher = await getMatcher(projectPath);
      debugLog('[searchFilesByPageName] getMatcher 成功，开始 matchFilesFromText...');
      const result = matcher.matchFilesFromText(pageName);
      debugLog('[searchFilesByPageName] matchFilesFromText 完成: method=' + result.method + ' files=' + result.files.length);

      if (result.files.length > 0) {
        console.log('[searchFilesByPageName] DynamicFileMatcher 命中: method=' + result.method + ' 文件数=' + result.files.length);
        result.trace.forEach(t => console.log('  ' + t));

        // 检查文件是否存在，过滤掉不存在的
        const validFiles = result.files.filter(f => fs.existsSync(f.path));
      ...
      console.log('[searchFilesByPageName] DynamicFileMatcher 未命中，回退到代码图搜索');
      result.trace.forEach(t => console.log('  ' + t));
    } catch (matcherError) {
      debugLog('[searchFilesByPageName] DynamicFileMatcher 失败: ' + matcherError.message + '\n' + matcherError.stack);
      console.warn('[searchFilesByPageName] DynamicFileMatcher 失败:', matcherError.message);
    }
    ...
    try {
      ...
      console.warn('[searchFilesByPageName] 加载代码图失败:', e.message);
    }
    ...
    searchKeywords.add(pageName.toLowerCase().replace(/\s+/g, '_'));
    searchKeywords.add(pageName.toLowerCase().replace(/\s+/g, '-'));
    // 驼峰转换
    const camelCase = pageName.toLowerCase().replace(/[^a-z0-9]+(.)/g, (_, c) => c.toUpperCase());
    searchKeywords.add(camelCase);
    // 提取中文短语（2-6字）作为关键词
    const chinesePhrases = pageName.match(/[一-鿿]{2,6}/g) || [];
    chinesePhrases.forEach(p => searchKeywords.add(p));

    console.log('[searchFilesByPageName] 回退关键词:', [...searchKeywords].join(', '));
    ...
    for (const node of codeGraph.nodes) {
      if (node.type === 'file' && node.filePath) {
        fileNameToPath.set(node.fileName, node.filePath);
      }
    }
    ...
    for (const node of codeGraph.nodes) {
      ...
      const isMatch = [...searchKeywords].some(kw =>
        fileName.includes(kw) || filePath.includes(kw.replace(/\s+/g, '_'))
      );

      if (isMatch) {
        const fileType = inferFileType(node.filePath, projectType);
        if (['view', 'controller', 'api', 'service', 'model'].includes(fileType)) {
          matchedFiles.set(node.filePath, fileType);
        }
      }
    }

    // 依赖扩展：从 view/controller 找 import 的关联文件
    const expandedFiles = new Map(matchedFiles);
    for (const [fPath] of matchedFiles) {
      try {
        if (!fs.existsSync(fPath)) continue;
        const fContent = fs.readFileSync(fPath, 'utf-8');
        const imports = parseImports(fPath, fContent);
        for (const imp of imports) {
          const resolved = fileNameToPath.get(path.basename(imp)) ||
            (fs.existsSync(imp) ? imp : null);
          if (resolved && fs.existsSync(resolved)) {
            const depType = inferFileType(resolved, projectType);
            if (!['test', 'generated', 'i18n'].includes(depType)) {
              expandedFiles.set(resolved, depType);
            }
          }
        }
      } catch (e) { /* 跳过不可读文件 */ }
    ...
    const resultFiles = Array.from(expandedFiles.entries())
      .filter(([p]) => fs.existsSync(p))
      .map(([p, t]) => ({ path: p, name: path.basename(p), type: t }));

    console.log('[searchFilesByPageName] 回退结果: 文件数=' + resultFiles.length);
    debugLog('[searchFilesByPageName] 回退结果: 文件数=' + resultFiles.length);
    return {
      success: true,
      files: resultFiles,
      projectType,
      method: 'codegraph-fallback'
    };
  } catch (error) {
    console.error('[searchFilesByPageName] 失败:', error.message, error.stack);
    return { success: false, error: error.message };
  }
});
